...
9. Confirm `candidate_pipeline.summary.screening_complete=true` also holds, then follow `nightly_research.md` for Event Research: run `init-event-research`, delegate the Web research (JPX earnings schedule, TDnet, issuer disclosures, Yahoo! news, Kabutan news) that fills Source Attempts, Evidence, and News Classification into `sources.json` and `event_research.json`, and restrict candidates to `status=ELIGIBLE` and `screening_status=PASS` only. Event Research records evidence and classifications; it must not decide `PASS`, `REJECT`, or `DATA_UNAVAILABLE`.
10. Run `validate-event-research`, then `build-event-gate` to produce `event_gate.json` exactly as `nightly_research.md` specifies. Confirm `event_gate.json` reports `event_gate_complete=true`.
11. Only proceed toward Ranking when `event_gate.json` reports `ranking_ready=true`. Ranking is not implemented in this repository. While Ranking is unimplemented, the main agent must not compare Event Gate `PASS` candidates itself and must not produce a `TRADE` recommendation, even when `PASS` candidates exist; write `NO_TRADE` (or `DATA_UNAVAILABLE` when required data is missing) per `nightly_research.md`. Never pass `REJECT` or `DATA_UNAVAILABLE` Event Gate candidates onward.
...
14. Complete the Risk Engine, `research.md`, `recommendation.md`, `report.md`, `official_ohlcv_audit.json`, run artifact allowlist validation (including `event_research.json` and `event_gate.json`), and recommendation recording exactly as specified by the nightly prompt.
