...
6. After Stage 2 Candidate Research is merged and **before running `screen-market`**, perform the Ranking Actual Turnover Research described in `nightly_research.md` for every Stage 1 `PASS` candidate: research the previous trading day's actual turnover from `YAHOO_JP_QUOTE`. Before requesting a `YAHOO_JP_QUOTE` URL, confirm from already-verified Source Evidence that the candidate is listed on the Tokyo Stock Exchange, because the URL template hardcodes the `.T` suffix and Discovery is `ALL_MARKETS`; if TSE listing cannot be confirmed, do not guess the suffix, do not record a `FOUND` attempt, and do not carry that candidate into Ranking. On `FOUND`, save the Source Attempt and the matching source record into `sources.json` and the same canonical turnover into `market_data.json.turnover`. On any failure status, save the failure Source Attempt as-is without inventing a substitute value, and set `market_data.json.turnover` to `null` so no stale `FOUND` value survives. Only then continue to Python validation and screening.
