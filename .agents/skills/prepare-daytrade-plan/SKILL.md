...
5. Save confirmed Stage 1 evidence and run `apply-stage1`. **Before Stage 2 Candidate Research begins** (Stage 2 can query `YAHOO_JP_HISTORY` and `YAHOO_JP_NEWS`, not just `YAHOO_JP_QUOTE`), confirm for every Stage 1 `PASS` candidate — from already-verified Source Evidence — that it is listed on the Tokyo Stock Exchange, because the `YAHOO_JP_HISTORY` / `YAHOO_JP_NEWS` / `YAHOO_JP_QUOTE` URL templates all hardcode the `.T` suffix and Discovery is `ALL_MARKETS`. If TSE listing cannot be confirmed, do not construct or fetch a guessed `.T` URL for any of these three sources, do not record a `FOUND` attempt for it, and do not carry that candidate into Stage 2 Candidate Research (and therefore not into Turnover Research, Event Research, or Ranking) — fail closed at this point. Only then run `plan-stage2-batches` and delegate bounded Stage 2 Candidate Research batches to `market_researcher` when available, restricted to candidates whose TSE listing was confirmed. Require standard source_checks and subagent merge state. Wait for its summary; the subagent must not write files.
6. After Stage 2 Candidate Research is merged and **before running `screen-market`**, perform the Ranking Actual Turnover Research described in `nightly_research.md` for every Stage 1 `PASS` candidate whose TSE listing was already confirmed in step 5: research the previous trading day's actual turnover from `YAHOO_JP_QUOTE`. On `FOUND`, save the Source Attempt and the matching source record into `sources.json` and the same canonical turnover into `market_data.json.turnover`. On any failure status, save the failure Source Attempt as-is without inventing a substitute value, and set `market_data.json.turnover` to `null` so no stale `FOUND` value survives. Only then continue to Python validation and screening.
