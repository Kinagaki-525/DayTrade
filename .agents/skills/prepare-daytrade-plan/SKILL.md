...
3. Have the main agent follow the nightly prompt to snapshot config, resolve and save the Python-generated research window, and and run `acquire-discovery`, which fetches both Yahoo ranking pages by curl GET, stores the raw bytes with their SHA256, parses them deterministically, confirms TOP50, and writes `market_research.json`. Discovery candidates are never typed in by an agent or a subagent.
