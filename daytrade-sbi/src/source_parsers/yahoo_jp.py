...
def parse_history(
    ...
    try:
        previous_row = _previous_trading_day_row(
            table_rows(page.text), context.trading_date, _iso_date
        )
    except ParseFailed as exc:
        return parse_failed(exc.message)
...
def _previous_trading_day_row(
    ...
    itself publishes -- never by row position -- so a page whose ordering
    does not match assumptions still resolves correctly, and a page that
    omits the previous session simply yields no previous-day values instead
    of a guess.
    """
