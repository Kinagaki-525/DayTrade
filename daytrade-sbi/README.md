...
Hard Screening PASS候補と分析Featureを保存
  ...
  ↓ Case C（Selection有効時）: `build-selection`でRank 1のみを評価（Rank2フォールバックなし）→`build-selection-recommendation`でrecommendation.jsonを確定。`build-selection`・`build-selection-recommendation`・`risk-check`はそれぞれ独立に`ranking.json`の上流Trust Chain（Event Gate・Candidates・Market Data・Sources・Source Matrix・Config Snapshot）を再検証するため、Selection/RecommendationのSHA256ハッシュチェーンが自己整合的なだけでは不十分
