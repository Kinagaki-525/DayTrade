...
- Trust Boundary（Case C）: `selection.json`/`recommendation.json`自身のSHA256ハッシュチェーンが一致しているだけでは足りない。`build-selection`・`build-selection-recommendation`・`risk-check`の3つはそれぞれ独立に、共有ヘルパー`load_and_verify_ranking_trust_chain`（`src/ranking_trust.py`）経由で`ranking.json`をその上流アーティファクト一式（`event_gate.json`・`candidates.json`・`market_data.json`・`sources.json`・source matrix・`strategy_snapshot.yaml`）に対して再検証してから初めてSelection/Recommendationの内容を信用する。agentが`ranking.json`を編集し、そこから自己整合的な`selection.json`/`recommendation.json`を再生成しても、Case Cの有効な結果は得られない。
