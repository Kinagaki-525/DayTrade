...
- Discovery未完了（`acquire-discovery`のCLI resultが`status=CLOSED`、または`market_research.overall_status=DISCOVERY_INCOMPLETE`）は`NO_TRADE`ではない。候補が無かったのではなく候補Universeを生成できなかった状態であり、`Pipeline: 未完了`／`停止Stage: DISCOVERY`として報告する。`candidate_count == 0`だけを理由に`NO_TRADE`へ変換しない。なお`acquire-discovery`のexit codeが非0の場合は、後続Stageへ進まずSTOPする
- Discovery未完了でPipelineが停止した場合、後続のRecommendation / Risk Builderを実行して日次結果を新規生成・補完しない。agentが`recommendation.json`・`risk_result.json`を手書きして日次結果を補完してはいけない。Discovery Fail-Closedの証跡（`market_research.json`・`sources.json`・`network_requests/`・`source_attempts[]`）はそのまま保持し、後続Stageへ進まない
